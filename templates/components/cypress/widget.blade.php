    ...
    <div class="flex flex-col md:flex-row">
        ...
        <div class="md:w-1/3 mb-4 md:mb-0">
            @foreach ($items as $item)
                <x-bonsai::cypress.bonsai::cypress.bonsai::cypress.bonsai::cypress.bonsai::cypress.bonsai::cypress.bonsai::cypress.bonsai::accordion :data="['item' => $item]" />
            @endforeach
        </div>
        ...
        <div class="md:w-2/3">
            @foreach ($items as $item)
                <div x-show="activeAccordion === '{{ $item['id'] }}'" class="{{ $contentClasses }}">
                    ...
                    @endphp
                    <x-bonsai::cypress.bonsai::cypress.bonsai::cypress.bonsai::cypress.bonsai::cypress.bonsai::cypress.bonsai::cypress.bonsai::cta :data="$ctaData" />
                    ...
                    @if(isset($item['listItems']) && is_array($item['listItems']))
                        <div class="grid md:grid-cols-2 gap-4 mt-4">
                            @foreach ($item['listItems'] as $listItem)
                                ...
                                @endphp
                                <x-bonsai::cypress.bonsai::cypress.bonsai::cypress.bonsai::cypress.bonsai::cypress.bonsai::cypress.bonsai::cypress.bonsai::list-item :data="$listItem" />
                            @endforeach
                        </div>
                    @endif
